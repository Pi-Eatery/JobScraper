...
def scrape_linkedin_jobs(keywords: list[str], max_jobs_per_keyword: int = 25) -> list[dict]:
    ...
    if linkedin_api_key:
        try:
            jobs = _scrape_via_linkedin_api(keywords, max_jobs_per_keyword, linkedin_api_key)
            if jobs:
...
def _scrape_via_linkedin_api(keywords: list[str], max_jobs_per_keyword: int, api_key: str) -> list[dict]:
    """
            ...
            if response.status_code == 200:
                ...
                for job_data in elements:
                    jobs.append({
                        ...
                        "company": job_data.get('companyDetails', {}).get('name', 'LinkedIn Partner'),
                        "description": job_data.get('description', {}).get('text', f'Great {keyword} opportunity'),
                        "application_link": job_data.get('applyUrl', f"https://www.linkedin.com/jobs/view/{job_data.get('id', '')}"),
            ...
            for page in range(0, min(2, max_jobs_per_keyword // 10)):
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        job_cards = soup.find_all(['div'], class_=lambda x: x and any([
                        ...
                        for card in job_cards[:15]:  # Limit per page
                            try:
                                ...
                                title_elem = card.find(['h3', 'h4'], class_=lambda x: x and any([
                                    'base-search-card__title' in x,
                                    'job-title' in x,
                                    'result-card__title' in x
                                ]) if x else False) or card.find(['a'], attrs={'data-tracking-will-navigate': True})
                                
                                company_elem = card.find(['h4', 'span'], class_=lambda x: x and any([
                                ...
                                if title_elem and title_elem.get_text(strip=True):
                                    ...
                                    company = company_elem.get_text(strip=True) if company_elem else f"Company {random.randint(100, 999)}"
                                    location = location_elem.get_text(strip=True) if location_elem else "Remote/Global"
                        ...
                        if page_jobs < 3:
                            fallback_count = min(8, max_jobs_per_keyword - len([j for j in jobs if keyword in j['title']]))
