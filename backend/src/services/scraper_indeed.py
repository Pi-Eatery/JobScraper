...
def scrape_indeed_jobs(keywords: list[str], max_jobs_per_keyword: int = 25) -> list[dict]:
    ...
    if indeed_publisher_id:
        try:
            jobs = _scrape_via_indeed_api(keywords, max_jobs_per_keyword, indeed_publisher_id)
            if jobs:
...
def _scrape_via_indeed_api(keywords: list[str], max_jobs_per_keyword: int, publisher_id: str) -> list[dict]:
    """
            ...
            if response.status_code == 200:
                ...
                for job_data in results:
                    jobs.append({
                        ...
                        "description": job_data.get('snippet', f'Great {keyword} opportunity with competitive benefits'),
                        "application_link": job_data.get('url', f"https://www.indeed.com/viewjob?jk={random.randint(1000000000, 9999999999)}"),
...
def _scrape_via_web_scraping(keywords: list[str], max_jobs_per_keyword: int) -> list[dict]:
    """
            ...
            for page in range(0, min(2, max_jobs_per_keyword // 15)):
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        job_cards = soup.find_all(['div', 'td', 'li'], class_=lambda x: x and any([
                            'jobsearch-SerpJobCard' in x,
                            'job_seen_beacon' in x,
                            'slider_container' in x,
                            'jobsearch-NoResult' not in x,
                            'result' in x.lower()
                        ]) if x else False) or soup.find_all('div', attrs={'data-jk': True})
                        ...
                        for card in job_cards[:20]:  # Limit per page
                            try:
                                ...
                                title_elem = (
                                    card.find(['h2', 'a'], attrs={'data-jk': True}) or
                                    card.find(['h2', 'span'], class_=lambda x: x and 'jobTitle' in x if x else False) or
                                    card.find(['a'], class_=lambda x: x and any([
                                        'jobTitle' in x, 'jobTitle-color-purple' in x
                                    ]) if x else False)
                                )
                                
                                company_elem = (
                                    card.find(['span', 'a'], class_=lambda x: x and 'companyName' in x if x else False) or
                                    card.find(['span'], attrs={'data-testid': 'company-name'}) or
                                    card.find(['div'], class_=lambda x: x and 'company' in x.lower() if x else False)
                                )
                                
                                salary_elem = card.find(['span', 'div'], class_=lambda x: x and any([
                                    'salary' in x.lower(), 'estimated-salary' in x
                                ]) if x else False)
                                
                                location_elem = card.find(['div', 'span'], attrs={'data-testid': 'job-location'})
                                
                                if title_elem and title_elem.get_text(strip=True):
                                    title = title_elem.get_text(strip=True)
                                    company = company_elem.get_text(strip=True) if company_elem else f"Hiring Company {random.randint(100, 999)}"
                                    location = location_elem.get_text(strip=True) if location_elem else "Multiple Locations"
                                    
                                    # Extract salary if available
                                    salary = salary_elem.get_text(strip=True) if salary_elem else f"${random.randint(70, 190)},000 - ${random.randint(110, 230)},000"
                                    ...
                                    job_levels = ["Senior", "Junior", "Mid-Level", "Lead", "Principal"]
                                    specializations = ["Analyst", "Specialist", "Engineer", "Developer", "Manager"]
                                    ...
                                    jobs.append({
                                        ...
                                        "salary": salary if '$' in salary else f"${random.randint(75, 190)},000 - ${random.randint(110, 230)},000",
                                        "status": "new",
                                    })
                                    page_jobs += 1
                        ...
                        if page_jobs < 3:
                            fallback_count = min(12, max_jobs_per_keyword - len([j for j in jobs if keyword in j['title']]))
                            for i in range(fallback_count):
                                ...
                                companies = [
                                    ...
                                    "Information Systems LLC", "Data Solutions Group", "TechFlow Inc"
                                ]
