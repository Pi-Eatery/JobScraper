...
def db_session(setup_database, request):
    ...
    user = User(
        username=unique_username, email=unique_email, password_hash="hashedpassword"
    )
    session.add(user)
    session.commit()
...
def test_get_applications(db_session, application_service, test_user):
    ...
    applications_limit_1 = application_service.get_applications(
        db_session, test_user.id, limit=1
    )
...
def test_get_application(db_session, application_service, test_user, request):
    created_app = application_service.create_application(
        db_session,
        test_user.id,
        "Single App",
        "Single Comp",
    ...
    found_app = application_service.get_application(
        db_session, created_app.id, test_user.id
    )
    ...
    other_user = User(
        username=f"otheruser_{request.node.name}",
        email=f"other_{request.node.name}@example.com",
        password_hash="hash",
    )
    db_session.add(other_user)
    db_session.commit()
    db_session.refresh(other_user)
    not_found_app_other_user = application_service.get_application(
        db_session, created_app.id, other_user.id
    )
...
def test_update_application(db_session, application_service, test_user):
    ...
    not_found_updated_app = application_service.update_application(
        db_session, 999, test_user.id, job_title="Non Existent"
    )
...
def test_delete_application(db_session, application_service, test_user):
    ...
    deleted_app = application_service.delete_application(
        db_session, created_app.id, test_user.id
    )
    ...
    assert (
        ...
        .filter(JobApplication.id == created_app.id)
        .first()
        is None
    )

    not_found_deleted_app = application_service.delete_application(
        db_session, 999, test_user.id
    )
