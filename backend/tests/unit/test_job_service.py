...
def test_get_job(mock_db_session):
    ...
    mock_db_session.query.return_value.filter.return_value.first.return_value = Job(
        id=job_id, title="Job 1", company="Comp 1", user_id=user_id
    )
...
def test_update_job_status(mock_db_session):
    ...
    new_status = "applied"
    mock_job = Job(
        id=job_id, title="Job 1", company="Comp 1", user_id=user_id, status="new"
    )
    ...
    updated_job = job_service.update_job_status(
        mock_db_session, job_id, user_id, new_status
    )
